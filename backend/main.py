...
# =====================================================================
# THÊM MỚI: Hàm AI tính toán độ khả thi dự án
# =====================================================================
def generate_ai_diagnosis(avg_cycle, sprint_health):
    score = 100
    reasons = []
    
    if avg_cycle > 4:
        score -= 20
        reasons.append(f"🔴 Thời gian code (Cycle Time) khá lâu ({round(avg_cycle, 1)} ngày). Dự án dễ fail vì task chia quá to hoặc Dev đang bị block.")
    else:
        reasons.append("🟢 Tốc độ xử lý (Cycle Time) đang ở mức tốt.")

    if "RỦI RO" in sprint_health:
        score -= 30
        reasons.append("🔴 Phát hiện nhiều task có rủi ro trễ hạn. Tốc độ làm việc đang chậm hơn yêu cầu thực tế.")
    
    if score == 100:
        reasons.append("🟢 Dự án rất tốt: Tốc độ xử lý nhanh, an toàn và đang đi đúng hướng (On Track).")

    return {
        "score": score,
        "status": "Khả thi" if score >= 80 else "Có nguy cơ Fail",
        "reasons": reasons
    }

# ========== CÁC HÀM TÍNH TOÁN (Giữ nguyên của bạn) ==========
def estimate_story_points(issue):
    for label in issue.get('labels', []):
        if isinstance(label, dict) and label.get('name', '').upper().startswith('SP:'):
            try: return int(label.get('name').split(':')[1].strip())
            except: pass
    title_upper = issue['title'].upper()
    labels_lower = [l['name'].lower() for l in issue.get('labels', []) if isinstance(l, dict)]
    if '[BUG]' in title_upper or 'bug' in labels_lower: base_sp = 2
    elif '[DOCS]' in title_upper or 'documentation' in labels_lower: base_sp = 1
    elif '[FEAT]' in title_upper or 'feature' in labels_lower: base_sp = 5
    elif '[ENHANCE]' in title_upper: base_sp = 5
...
def analyze_nlp_description(issue):
    ...
    if len(body.strip()) < 20: return "⚠️ Mô tả quá ngắn", False
    return "✅ Chi tiết", True
...
def calculate_burndown_data(issues, start_project_date):
    ...
    if all_dates:
        ...
        tasks_remaining = total_created_sp 
        tasks_closed_cum = 0
...
# ========== MAIN ENDPOINT ĐÃ ĐỒNG BỘ VỚI FRONTEND V6 ==========
...
async def get_agile_metrics(owner: str, repo: str):
    ...
    if not issues: raise HTTPException(status_code=404, detail="Khong tim thay du lieu")
    ...
    for issue in issues:
        ...
        task_base = {
            ...
            "nlpStatus": nlp_status, "dateOpen": created_at.strftime("%d/%m/%Y"),
        ...
        if issue['state'] == 'closed' and issue['closed_at']:
            ...
            if risk_model:
                try:
                    # Tạo DataFrame 1 dòng có chứa TÊN CỘT để AI hết cằn nhằn
                    input_data = pd.DataFrame(
                        [[cycle_time, sp, random.randint(0, 3)]], 
                        columns=['cycle_time', 'sp', 'bug_count']
                    )
    ...
    sprint_health = "CÓ RỦI RO" if not df.empty and (high_risk_tasks / len(df)) > 0.3 else "AN TOÀN"
    available_sprints = sorted(list(set(item['sprint'] for item in burndown_data)), key=lambda x: int(x.split()[1]))

    # Gọi AI Đánh giá
    diagnosis = generate_ai_diagnosis(avg_cycle, sprint_health)

    return {
        "velocity": total_velocity, "avgCycleTime": round(avg_cycle, 2), "avgLeadTime": round(avg_lead, 2),
        "teamProductivity": productivity_data["teamProductivity"], "tasksPerDay": productivity_data["tasksPerDay"],
        "burndownData": burndown_data, "availableSprints": available_sprints,
        "sprintHealth": sprint_health, "tasks": tasks, "kanban": kanban,
        "memberPerformance": list(members_data.values()),
        "projectDiagnosis": diagnosis # Trả về cho Chatbox
    }
